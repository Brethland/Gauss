...
def mult(M1: M, M2: M) -> M:
    ...
    return [[sum(x * y for x, y in zip(row, column(M2, c))) for c in range(len(M2[0]))] for row in M1]

def blocking(M1: M, b: int) -> tuple[M, M, M, M]:
    return ([n[:b] for n in M1[:b]],
            [n[b:] for n in M1[:b]],
            [n[:b] for n in M1[b:]],
            [n[b:] for n in M1[b:]])
...
def mult_strassen(M1: M, M2: M) -> M:
    ...
    K6 = mult_strassen(add_iterative(A21, scalar_mult(A11, -1)), add_iterative(B11, B12))
    K7 = mult_strassen(add_iterative(A12, scalar_mult(A22, -1)), add_iterative(B21, B22))
